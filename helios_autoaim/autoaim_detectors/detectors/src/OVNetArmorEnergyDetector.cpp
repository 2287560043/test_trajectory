...
const std::vector<std::string> ENERGY_NUMBER_LABEL{ /// TODO: add energy number label
                                                    "energy_target", "energy_fan", "energy_r"
};
...
void YOLOXDetector::initialize(const std::string& model_path, const OVNetArmorEnergyDetectorParams& params)
{
  ...
  try
  {
    ...
    if (std::find(available_devices.begin(), available_devices.end(), "GPU") != available_devices.end())
    {
      target_device = "GPU";
    }
    ...
    ov::Tensor dummy_tensor = ov::Tensor(ov::element::u8, {static_cast<size_t>(1), static_cast<size_t>(INPUT_H), static_cast<size_t>(INPUT_W), static_cast<size_t>(3)}, dummy.data);
...
void YOLOXDetector::draw_objects(cv::Mat& frame, const ArmorsStamped& armors_stamped)
{
  for (const auto& armor : armors_stamped.armors)
  {
    ...
    {
      cv::line(frame, armor.left_light.top, armor.right_light.bottom, cv::Scalar(0, 255, 0), 2);
      cv::line(frame, armor.left_light.bottom, armor.right_light.top, cv::Scalar(0, 255, 0), 2);
  ...
  for (const auto& armor : armors_stamped.armors)
  {
    if (armor.type != ArmorType::INVALID)
    {
      cv::putText(frame, armor.number, armor.left_light.top, cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 255), 2);
      cv::putText(frame, std::to_string(armor.confidence), armor.right_light.top, cv::FONT_HERSHEY_SIMPLEX, 0.8,
  ...
  for (const auto& armor : armors_stamped.armors)
  {
    if (armor.type != ArmorType::INVALID) 
    {
      auto check_point = [&frame](const cv::Point2f& p) -> bool {
        return p.x >= 0 && p.x < frame.cols && 
               p.y >= 0 && p.y < frame.rows &&
...
ArmorType YOLOXDetector::judge_armor_type(const Armor& armor)
{
  ...
  return center_distance > params_.min_large_center_distance ? ArmorType::LARGE : ArmorType::SMALL;
}
std::pair<std::vector<Armor>, rclcpp::Time> YOLOXDetector::sync_detect_with_timestamp(const cv::Mat& frame, const rclcpp::Time& timestamp)
{
  ...
  float scale = std::min(static_cast<float>(INPUT_W) / frame.cols, static_cast<float>(INPUT_H) / frame.rows);
  ...
  ov::Tensor input_tensor = ov::Tensor(ov::element::u8, {1, static_cast<size_t>(INPUT_H), static_cast<size_t>(INPUT_W), 3}, input_frame.data);
  ...
  for (const auto& object : objects)
  {
    ...
    if (params_.autoaim_mode != 0)
    {
      ...
      armor_target.type = object.label == 0 ? ArmorType::ENERGY_TARGET : ArmorType::ENERGY_FAN;
    }
    else
    {
      ...
      // 计算中心点(使用映射后的坐标)
      armor_target.center = (armor_target.left_light.top + 
                            armor_target.left_light.bottom + 
                            armor_target.right_light.top + 
                            armor_target.right_light.bottom) * 0.25f;
  ...
  current_latency_ms_ = std::chrono::duration<double, std::milli>(processing_end - processing_start).count();
  ...
  if (params_.debug && !frame.empty())
  {
    ...
    cv::putText(debug_frame, ss_latency.str(), cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 255, 0), 2);
...
void YOLOXDetector::generate_yolox_proposal(const std::vector<GridAndStride>& grid_strides, const float* output_buffer,
                                            std::vector<Object>& objects)
{
...
void YOLOXDetector::stop_processing()
{
  ...
  {
    RCLCPP_ERROR(rclcpp::get_logger("YOLOXDetector"), "Error stopping processing: %s", e.what());
  }
}
...
void YOLOXDetector::update_params(const OVNetArmorEnergyDetectorParams& new_params) {
    ...
    if (significant_change) {
        RCLCPP_INFO(rclcpp::get_logger("YOLOXDetector"), "Significant parameter change detected, reinitializing detector");
        stop_processing();
        initialize(new_params.net_params.MODEL_PATH, new_params);
        start_processing();
    } else {
        RCLCPP_INFO(rclcpp::get_logger("YOLOXDetector"), "Updating parameters without reinitialization");
        params_ = new_params;
    }
}
...
{
  std::sort(objects.begin(), objects.end(), [](const Object& a, const Object& b) { return a.conf > b.conf; });
}
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    for (int j = 0; j < (int)picked.size(); j++)
    {
      ...
      if (iou > NMS_THRESH || std::isnan(iou))
      {
        ...
        {
          faceobjects[picked[j]].apexes.insert(faceobjects[picked[j]].apexes.end(), a.apexes.begin(), a.apexes.end());
        }
        break;
      }
    }
...
void YOLOXDetector::generate_grids_and_stride(const int w, const int h, const int strides[],
...
std::shared_ptr<Armors> OVNetArmorEnergyDetector::detect_armors(std::shared_ptr<Image> img_stamped)
{
  auto armors_stamped = std::make_shared<ArmorsStamped>();
  std::shared_ptr<ImageStamped> image_stamped = std::dynamic_pointer_cast<ImageStamped>(img_stamped);
  ...
  auto [armors, timestamp] = detector_->sync_detect_with_timestamp(image_stamped->image, original_timestamp);
...
void OVNetArmorEnergyDetector::set_params(void* params)
{
  ...
  if (params_changed)
  {
    try
    {
      detector_ = std::make_unique<YOLOXDetector>(new_params.net_params.MODEL_PATH, new_params);
    }
    catch (const std::exception& e)
    {
      RCLCPP_ERROR(rclcpp::get_logger("OVNetArmorEnergyDetector"), "Failed to update detector parameters: %s", e.what());
      return;
    }
  }
...
DebugImages* OVNetArmorEnergyDetector::get_debug_images()
{
  debug_images_.result_img = detector_->get_debug_image();
  return &debug_images_;
}

DebugInfos* OVNetArmorEnergyDetector::get_debug_infos()
{
  return &debug_infos_;
}
}  // namespace helios_cv