...
namespace helios_cv {
    ...
    void Camera::processFrame(std::shared_ptr<CameraFrame> cameraFrame, BYTE * output) {
    ...
    bool Camera::startCamera() {
        if (CameraSetCallbackFunction(h_camera_, frameCallback, this, nullptr)
    ...
    void Camera::frameCallback(CameraHandle hCam, BYTE * buf, tSdkFrameHead * head, PVOID ctx) {
        ...
        cam->frameCallback_(std::make_shared<CameraFrame>(head, buf, frame_timestamp_ms));
    }

    template<typename... Args>
    void Camera::log(const LogLevel logLevel, const std::string& fmt, Args... args) {
        if (logCallback_)
