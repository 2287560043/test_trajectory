...
void DetectorNode::armor_image_callback(sensor_msgs::msg::Image::SharedPtr image_msg) {
    ...
    auto image_time = rclcpp::Time(image_msg->header.stamp);
    auto receive_latency = (receive_time - image_time).nanoseconds() / 1e6;
    
    RCLCPP_INFO_THROTTLE(logger_, *this->get_clock(), 1000,
        "Direct subscription latency: %.2f ms", receive_latency);
