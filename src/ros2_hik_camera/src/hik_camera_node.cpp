    ...
    capture_thread_ = std::thread{[this]() -> void {
      ...
      while (rclcpp::ok()) {
        ...
        if (MV_OK == nRet) {
          ...
          image_msg_->data.resize(OutFrame.stFrameInfo.nWidth * OutFrame.stFrameInfo.nHeight * 3);
          ...
          RCLCPP_ERROR(this->get_logger(), "Get buffer failed! nRet: [%x]", nRet);
...
private:
  void declareParameters()
  {
    ...
    // Flip image 
    rcl_interfaces::msg::ParameterDescriptor flip_desc;
    flip_desc.description = "Enable/disable image flip";
    flip_image_ = this->declare_parameter("flip_image", false, flip_desc);
    RCLCPP_INFO(this->get_logger(), "Flip image: %s", flip_image_ ? "true" : "false");
    
    if (flip_image_) {
      MV_CC_SetBoolValue(camera_handle_, "ReverseY", true);
      MV_CC_SetBoolValue(camera_handle_, "ReverseX", true);
    }else{

      MV_CC_SetBoolValue(camera_handle_, "ReverseY", false);
      MV_CC_SetBoolValue(camera_handle_, "ReverseX", false);
    }
  }
    ...
    const std::vector<rclcpp::Parameter> & parameters)
  {
    ...
    for (const auto & param : parameters) {
      if (param.get_name() == "exposure_time") {
        ...
        if (flip_image_) {
          FlipParam_.nWidth = img_info_.nWidthMax;
          FlipParam_.nHeight = img_info_.nHeightMax;
          FlipParam_.enPixelType = PixelType_Gvsp_RGB8_Packed;
          FlipParam_.enFlipType = MV_FLIP_VERTICAL; 
        }
      } else {
        result.successful = false;
  ...
  MV_CC_FLIP_IMAGE_PARAM FlipParam_;
  bool flip_image_;
